...
class RNArunner:
    ...
    def __init__(self, sequence: str) -> None:
        ...
        if not all(c.upper() in valid_nucleotides for c in sequence):
            raise RNAFoldError(
                f"Invalid nucleotides in sequence. Only A, U, G, C allowed."
            )
    ...
    async def mfe(self) -> Tuple[str, float]:
        ...
        try:
            ...
            if proc.returncode != 0:
                ...
                raise RNAFoldError(
                    f"RNAFold failed with return code {proc.returncode}: {error_msg}"
                )
            ...
            if len(result_lines) < 3:
                raise RNAFoldError(
                    f"Unexpected RNAFold output format: {stdout.decode()}"
                )
            ...
            if not structure_line or not energy_line:
                raise RNAFoldError(
                    f"Could not parse structure from output: {stdout.decode()}"
                )
    ...
    def __repr__(self) -> str:
        ...
        truncated = self.sequence[:20]
        suffix = "..." if len(self.sequence) > 20 else ""
        return f"RNArunner(sequence='{truncated}{suffix}')"
