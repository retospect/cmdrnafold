...
class TestCodeFormatting:
    ...
    @pytest.mark.skipif(
        sys.platform == "win32", reason="mypy can be unstable on Windows CI"
    )
...
class TestProjectStructure:
    ...
    def test_src_directory_exists(self):
        ...
        assert (
            SRC_DIR / "cmdrnafold" / "__init__.py"
        ).exists(), "__init__.py must exist"
                ...
                for node in ast.walk(tree):
                    if isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef)):
                        ...
                        if (
                            ...
                            violations.append(
                                f"{py_file}:{node.lineno}: "
                                f"{node.name} class missing docstring"
                            )
